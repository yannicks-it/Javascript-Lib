...
- [Module Guide](#module-guide)
  ...
  - [Smooth Scroll Menu State](#smooth-scroll-menu-state)
...
- Initialization helper: `UpdateMenu()` hides every content pane whose paired trigger is not already marked with `yk-state="visible"`. Call it after dynamically injecting menus.
...
- Refresh helper: `refreshScrollMenuList()` rebuilds the internal cache of scroll menu items. Call it after adding or removing `[yk-type=onClick][yk-option=scroll][yk-arg=menu]` elements.
...
- Refresh helper: `refreshAutoScrollList()` rebuilds the cached scroll metadata—run it after adding or removing auto-scroll containers or their children.

### Smooth Scroll Menu State
- `EnableAutoSmoothScroll` is `false` by default; set it to `true` to let scroll events automatically adjust which menu item is marked with `yk-state="visible"` when the previously active target becomes hidden.
...
- Static helpers:
  ...
  - `CreateNumberFilterWithRange(element, min, max)` – enforces integer-only input plus range bounds.
  - `CreateNumberFloatFilterWithRange(element, min, max)` – float variant with optional min/max.
...
- Additional helpers: `setFilterWithKeys(...chars)` creates a whitelist of allowed characters; `setFilterWithRegex(regex)` applies a regex per character; `currentVal()` returns the current value; `allowedControlKeys()`/`disableControlKeys()` toggle whether navigation keys bypass the filter; `isControlKeysEnabled()` checks that state.
